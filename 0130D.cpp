        ...
        for(int j=1;j<=n;j++)
        {
            if(dis[s1][i]+dis[i][j]+dis[j][t1]<=l1)
                if(dis[s2][i]+dis[i][j]+dis[j][t2]<=l2)
                    ans=max(ans,m-dis[i][j]-dis[s1][i]-dis[s2][i]-dis[j][t1]-dis[j][t2]);
            if(dis[s1][j]+dis[j][i]+dis[i][t1]<=l1)
                if(dis[s2][i]+dis[i][j]+dis[j][t2]<=l2)
                    ans=max(ans,m-dis[i][j]-dis[s1][j]-dis[s2][i]-dis[i][t1]-dis[j][t2]);
            if(dis[s1][i]+dis[i][j]+dis[j][t1]<=l1)
                if(dis[s2][j]+dis[j][i]+dis[i][t2]<=l2)
                    ans=max(ans,m-dis[i][j]-dis[s1][i]-dis[s2][j]-dis[j][t1]-dis[i][t2]);
            if(dis[s1][j]+dis[j][i]+dis[i][t1]<=l1)
                if(dis[s2][j]+dis[j][i]+dis[i][t2]<=l2)
                    ans=max(ans,m-dis[j][i]-dis[s1][j]-dis[s2][j]-dis[i][t1]-dis[i][t2]);
        }
    printf("%d\n",ans);
    return 0;
}